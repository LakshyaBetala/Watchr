...
export default function AlertsCenter() {
  const allAlerts = useDashboardStore(state => state.allAlerts);
  const [alerts, setAlerts] = useState<any[]>(allAlerts);
  
  useEffect(() => {
    setAlerts(prev => {
      const existing = new Set(prev.map(a => a.id));
      const newIncoming = allAlerts.filter((a: any) => !existing.has(a.id));
