...
function ThermalGrid({ heatmapZoneCells }: { heatmapZoneCells: any[] }) {
  const [cells, setCells] = useState(heatmapZoneCells);

  useEffect(() => {
    setCells(heatmapZoneCells);
  }, [heatmapZoneCells]);
...
export default function SafetySentinel() {
  const { fireSignalHistory, signalStrengths, heatmapZoneCells } = useDashboardStore();
  
  const fireActive = fireSignalHistory.length > 0 && fireSignalHistory[fireSignalHistory.length - 1].t === new Date().toLocaleTimeString() && fireSignalHistory[fireSignalHistory.length - 1].heat > 80;
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <motion.div variants={itemVar}
          ...
          <div className="flex-1 p-4">
            <ThermalGrid heatmapZoneCells={heatmapZoneCells} />
          </div>
          {/* Heatmap decay note */}
